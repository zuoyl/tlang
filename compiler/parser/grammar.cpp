...
Grammar::getNonterminalState(int index)
{
...
void Grammar::parseItems(string &ruleName, NFA **start, NFA **end) 
{
    // setup new state
...
void Grammar::parseAtom(string &ruleName, NFA **start, NFA **end) 
{
    if (isMatch(TT_OP, "(")) {
        advanceToken();
        parseAlternative(ruleName, start, end);
